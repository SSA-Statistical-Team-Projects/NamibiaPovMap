...
```{r}
hist_plot(dt = census_dt, var="rooms", weights = "wta_hh", col = "black")
```

```{r}
hist_plot(dt = survey_dt, var="rooms", weights = "wta_hh", col = "blue")
```

```{r}
hist_plot(dt = census_dt, var="hhsize", weights = "wta_hh", col = "brown")
```

```{r}
hist_plot(dt = survey_dt, var="hhsize", weights = "wta_hh", col = "red")
```

```{r}
vect=c("rooms", "hhsize")
for (i in 1:length(vect)){
print(hist_plot(dt=census_dt,var = vect[i], weights = "wta_hh",col = "black"))
}
```
...
yt2=cbind(survey_dt, yt)[c(new_varlist2, cand_varlist, "wta_hh", "cluster")]
```


```{r}
hist_plot(dt = dt2, var="cellphone" , weights = "wta_hh", col = "black")
```

```{r}
hist_plot(dt = dt2, var="wta_hh" , weights = "wta_hh", col = "black")
```
...
ebp_test_means(smp_data =as.data.frame(na.omit(yt2[,c(varlist,"wta_hh")])),
                  ...
                  varlist = varlist , smp_weights ="wta_hh",pop_weights = "wta_hh" )
...
nam_selvars_list <-
  countrymodel_select_stata(dt = na.omit(survey_dt[,c("lwel_PPP", "wta_hh",
                                                                 "ea_id",
                                                                 candvar_list)]),
                            xvars = candvar_list,
                            y = "lwel_PPP",
