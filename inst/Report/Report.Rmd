...
ebp_reportcoef_table(log_model, decimals = 3) %>%
  merge(variables_dt, by.x = "Variable", by.y = "var") %>%
...
ydump_dt <- readRDS("data-clean/estimation_results/welfareydump_dt")
...
Figure \@ref(fig-cv) shows the precision gains achieved of EBP estimation over reporting direct poverty rates. On average, EBP headcount poverty rate CVs are `r specify_decimal(gains_value, 2)` times less than the design effect CVs.
...
shp_dt %>%
  ggplot() + 
  geom_sf(aes(fill = Head_Count), size = 0.2) + # `geom_sf()` for spatial data
...
shp_dt %>%
  ggplot() + 
  geom_sf(aes(fill = Head_Count*censuspop), size = 0.2) +
  scale_fill_viridis(option = "A",  # Viridis color option
                     name = "Population \n of Poor",
                     direction = -1) +
  theme_minimal() # Minimal theme for a clean look
...
pov_dt <- 
  log_model$ind %>%
  merge(unique(census_dt[, c("region_code", "const_code", "region_name")] %>% 
...
newsurv_dt <- as.data.frame(na.omit(survey_dt[, c("wel_abs",
                                                  nam_selvars_list,
                                                  "new_const_code",
                                                  "wta_hh",
                                                  "region_prev_name",
...
newsurv_dt[, poor_abs := ifelse(poor_abs == "Poor", 1, 0)]


#### compute direct survey variance for each region
newsurv_obj <- survey::svydesign(ids = ~cluster, 
                                 weights = ~hhweight,
                                 strata = ~region_prev_code+region_prev_name,
                                 survey.lonely.psu = "adjust",
                                 data = newsurv_dt,
                                 nest = TRUE)

var_dt <- svyby(~poor_abs,
                ~region_prev_code+region_prev_name,
                newsurv_obj,
                svymean)

var_dt <- 
  var_dt %>%
  mutate(povLB = poor_abs - 1.96*se,
         povUB = poor_abs + 1.96*se) 
...
newsurv_dt[,!duplicated(colnames(newsurv_dt)), with = F] %>%
  ...
  summarize(direct_poverty = weighted.mean(x = poor_var, 
                                           w = wta_hh * hhsize, 
                                           na.rm = TRUE)) %>%
  merge(pov_dt %>%
...
##### create confidence bounds on the regional estimates

regpov_dt <- 
  regpov_dt %>%
  merge(var_dt[, c("region_prev_name", "povLB", "povUB")])

regpov_dt$region_prev_name[regpov_dt$region_prev_name == "caprivi"] <- "zambezi"

#### plot the direct vs ebp estimate at the regional level with the confidence bounds 
regpov_dt %>%
  mutate(direct_poverty = as.numeric(direct_poverty)) %>%
  mutate(ebp_poverty = as.numeric(ebp_poverty)) %>% 
  ggplot(aes(x = region_prev_name)) +
  geom_point(aes(y = ebp_poverty), color = "blue", size = 2) +  # Plotting provFH
  geom_errorbar(aes(ymin = povLB, ymax = povUB), width = 0.2, color = "red") +  # Error bars for DirectLB and DirectUB
